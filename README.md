...
標籤上的品名與顏色是從 EPB 的存貨名稱解析出來的。新品上市時若出現英文色名沒被翻成
中文、或想調整顯示字樣，不用改程式 —— 按查詢結果右上角的**「名稱設定」**即可。

- 查詢完再開設定，面板頂端會**自動列出本次查詢裡還沒中文化的顏色**，填好中文按「加入」。
- 四張表各管一件事：

  | 分頁 | 用途 | 例 |
  |---|---|---|
  | iPhone 顏色 | iPhone 英文色名 → 中文 | `COSMIC ORANGE` → 橙色 |
  | iPad／Mac 顏色 | iPad／Mac 英文色名或 3 字母縮寫 → 中文 | `SPACE GRAY`、`SPG` → 太空灰 |
  | 品名 | 解析出的品名再覆寫 | `Duo` → iPhone Duo |
  | 其他欄位 | 其餘顯示欄位整值取代 | Watch 錶殼色 `深古銅色` → 古銅 |

- 多字色名要寫完整（`STAR WHITE`），只寫 `STAR` 會對不到。
- 改完按**儲存**才會存檔，標籤預覽會立刻更新。
- iPad Pro 依既有設計保留英文色名，不受顏色表影響。

設定存在本機 `settings/label-overrides.json`（不入版控，更新程式不會被蓋掉）。
內建預設表在 `static/label-logic.js`，是全門市共用的底；設定檔只疊在它上面，
檔案不存在或損毀時自動退回內建預設，不影響標籤功能。
要把自己調好的設定分享出去，按「匯出 JSON」拿檔案。
